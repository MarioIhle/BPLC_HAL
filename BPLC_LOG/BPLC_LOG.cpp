...
void BPLC_log::tick()
{
    ...
    {
        const bool BUFFER_SLOT_NOT_EMPTY                = (bool)(this->eventBuffer[BUFFER_SLOT].loggedEvent != BPLC_EVENT_LOG__NO_EVENT_IN_THIS_SLOT);
...
void BPLC_log::logEvent(const e_BPLC_EVENT_t EVENT_TYPE, const uint8_t PAYLOAD, unsigned long TIMESTAMP)
{
    for(uint8_t BUFFER_SLOT = 0; BUFFER_SLOT < MAX_EVENTS_PER_CYCLE; BUFFER_SLOT++)
    {
        if(this->eventBuffer[BUFFER_SLOT].event == BPLC_EVENT_LOG__NO_EVENT_IN_THIS_SLOT)
        {
...
{
    Serial.println("##############################");  
    Serial.print("map object to REL11_CARD "); Serial.println(P_LOG->paylaod);
