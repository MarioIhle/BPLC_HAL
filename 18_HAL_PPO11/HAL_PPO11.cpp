...
bool HAL_PPO11::mapObjectToChannel(IO_Interface* P_IO_OBJECT, const e_EC_CHANNEL_t CHANNEL)
{
    ...
    else if(CHANNEL < EC_CHANNEL_1 || CHANNEL > PPO11_CHANNEL_COUNT)
    {
        this->setError(PPO11_ERROR__CHANNEL_OUT_OF_RANGE, __FILENAME__, __LINE__);
    }
    else if(!this->channels.isChannelFree((uint8_t)CHANNEL, PPO11_CHANNEL_COUNT))
    {
        ...
        {       
            if(this->channels.get(CH) != nullptr)
            {
                if(this->channels.get(CH)->newDataAvailable())
                {
                    ...
                    uint16_t TARGET_PWM_VALUE = map(this->channels.get(CH)->getHalData().analogIoData.value, 0, 255, 0, 4095);
