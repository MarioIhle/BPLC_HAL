...
{
    this->p_object[OUT_PORT_1]  = P_DO1;
    this->usedPorts     = 1;
...
{
    this->p_object[OUT_PORT_1]  = P_DO1;
    this->p_object[OUT_PORT_2]  = P_DO2;
    this->usedPorts     = 2;
...
HAL_FUSE11::HAL_FUSE11(const e_FUSE11_ADDRESS_t ADDRESS, Output* P_DO1, Output* P_DO2, Output* P_DO3)
{
    this->p_object[OUT_PORT_1]  = P_DO1;
    this->p_object[OUT_PORT_2]  = P_DO2;
    this->p_object[OUT_PORT_3]  = P_DO3;  
    this->usedPorts     = 3;
...
HAL_FUSE11::HAL_FUSE11(const e_FUSE11_ADDRESS_t ADDRESS, Output* P_DO1, Output* P_DO2, Output* P_DO3, Output* P_DO4)
{
    ...
    this->p_object[OUT_PORT_2]  = P_DO2;
    this->p_object[OUT_PORT_3]  = P_DO3;
    this->p_object[OUT_PORT_4]  = P_DO4;
    this->usedPorts     = 4;
...
void HAL_FUSE11::tick()
{
    ...
    if(this->to_fuseCheck.check())
    {
        ...
        {       
            const bool STATE = PCF.read(this->PIN.input[PORT]);     
            this->FUSE[PORT].setPortState(STATE);    
            f_somePinOfsomeDinCardChanged--;  
#ifdef DEBUG_HAL_FUSE11
...
Serial.println(" ");       
    ...
    {
        //OUTPUTs
        if(this->p_object[PORT]->isThereANewPortValue())    //Nur Wert abrufen und schreiben, falls dier sich geändert hat
        {            
            const s_portValue_t VALUE_TO_WRITE = this->p_object[PORT]->getValue();
                       
            if(VALUE_TO_WRITE.value >= 1)
            {
                PCF.write(this->PIN.output[PORT], false);
            }
            else if(VALUE_TO_WRITE.value == false)
            {
                PCF.write(this->PIN.output[PORT], true);
            }            
        }                      
    } 
}
