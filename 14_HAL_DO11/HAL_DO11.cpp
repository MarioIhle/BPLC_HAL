...
bool HAL_DO11::mapObjectToChannel(IO_Interface* P_IO_OBJECT, const e_EC_CHANNEL_t CHANNEL)
{
    ...
    else if(CHANNEL < EC_CHANNEL_1 || CHANNEL > DO11_CHANNEL_COUNT)
    {
        this->setError(DO11_ERROR__CHANNEL_OUT_OF_RANGE, __FILENAME__, __LINE__);
    }
    else if(!this->channels.isChannelFree((uint8_t)CHANNEL, DO11_CHANNEL_COUNT) && CHANNEL == DO11_CHANNEL_COUNT)
    {
        this->setError(DO11_ERROR__ALL_CHANNELS_ALREADY_IN_USE, __FILENAME__, __LINE__);
    }
    else if(!this->channels.isChannelFree((uint8_t)CHANNEL, DO11_CHANNEL_COUNT))
    {
        ...
        {       
            if(this->channels.get(CH) != nullptr)
            {
                if(this->channels.get(CH)->newDataAvailable())
                {
                    ...
                    const uint16_t APPLICATION_VALUE    = (uint16_t)this->channels.get(CH)->getHalData().analogIoData.value;
                    ...
                    {                    
                        case IO_TYPE__OUTPUT_PULL:
                            PCA.setChannelPWM(this->PIN[CH][LS_MOSFET], 0, TARGET_PWM_VALUE);        //lowSide
                            PCA.setChannelOff(this->PIN[CH][HS_MOSFET]);                             //highside
                            break;
                        
                        case IO_TYPE__OUTPUT_PUSH:
                            PCA.setChannelOff(this->PIN[CH][LS_MOSFET]);                             //lowSide
                            PCA.setChannelPWM(this->PIN[CH][HS_MOSFET], 0, TARGET_PWM_VALUE);        //highside
                            break;

                        case IO_TYPE__OUTPUT_PUSH_PULL:                            
                            //Um überschneidung bei umschalten der PWM zu vermeiden, sonst FETS = rauch :C
                            PCA.setChannelOff(this->PIN[CH][HS_MOSFET]); //Spannungsführend zuerst aus
                            PCA.setChannelOff(this->PIN[CH][LS_MOSFET]);
                            delay(1);       
                                                    
                            //FULL ON
                            if(TARGET_PWM_VALUE < DEAD_TIME)
                            {
                                PCA.setChannelOn(this->PIN[CH][LS_MOSFET]);
                            ...
                            else if(TARGET_PWM_VALUE > 4096 - DEAD_TIME)
                            {
                                ...
                                PCA.setChannelOn(this->PIN[CH][HS_MOSFET]);
                            }
                            //PWM
                            else
                            {                        
                                PCA.setChannelPWM(this->PIN[CH][LS_MOSFET],  TARGET_PWM_VALUE + DEAD_TIME,       4095);
                                PCA.setChannelPWM(this->PIN[CH][HS_MOSFET],  DEAD_TIME,               TARGET_PWM_VALUE);
                            }
                            break;
                        ...
                        case IO_TYPE__OUTPUT_PUSH_PULL_INVERT:                            
                            ...
                            {                        
                                PCA.setChannelPWM(this->PIN[CH][HS_MOSFET],  TARGET_PWM_VALUE + DEAD_TIME,       4095);
                                PCA.setChannelPWM(this->PIN[CH][LS_MOSFET],  DEAD_TIME,               TARGET_PWM_VALUE);
                            }
                            break;
                        ...
                        case IO_TYPE__SERVO:
                            ...
                            {                        
                                PCA.setChannelPWM(this->PIN[CH][LS_MOSFET],  TARGET_PWM_VALUE + DEAD_TIME,       4095);
                                PCA.setChannelPWM(this->PIN[CH][HS_MOSFET],  DEAD_TIME,               APPLICATION_VALUE);
                            }
                            break;
